...
units_test = [[32,32,32], [64,64,64], [32,32,32,32,32], [64,64,64,64,64], [32,32,32,32,32,32,32]]
...
for num_units in units: 
    #K.clear_session()
    filenameFig = 'Voltage_Bidirectional_GRU'
    ...
    if len(num_units) == 1:
        ...
        model.add(Bidirectional(CuDNNGRU(num_units[0]),
        ...
        for i in range(1,len(num_units) - 1):
            model.add(Bidirectional(CuDNNGRU(num_units[i],
                                 return_sequences=True)))
        
        
        model.add(Bidirectional(CuDNNGRU(num_units[-1])))
